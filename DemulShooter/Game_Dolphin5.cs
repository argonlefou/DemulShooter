...
namespace DemulShooter
{
    class Game_Dolphin5 : Game
    {
        ...
        protected string _KeybMouse_X_NOP_Offset;
        protected string _KeybMouse_X_NOP_Offset_2;
        protected string _KeybMouse_Y_NOP_Offset;
        protected string _KeybMouse_Y_NOP_Offset_2;
        ...
        /// Constructor
        /// </summary>
        public Game_Dolphin5(string RomName, int DinputNumber, bool Verbose)
            : base()
        {
            GetScreenResolution();
        ...
        {
            if (File.Exists(AppDomain.CurrentDomain.BaseDirectory + @"\" + FOLDER_GAMEDATA + @"\5.0_x64.cfg"))
            {
                using (StreamReader sr = new StreamReader(AppDomain.CurrentDomain.BaseDirectory + @"\" + FOLDER_GAMEDATA + @"\5.0_x64.cfg"))
                {
                    string line;
                    line = sr.ReadLine();
                    while (line != null)
                            ...
                            try
                            {
                                switch (buffer[0].ToUpper().Trim())
                                {
                                    ...
                                    case "KEYBMOUSE_RBTN_OFFSET":
                                        ...
                                        break;
                                    case "KEYBMOUSE_X_NOP_OFFSET":
                                        _KeybMouse_X_NOP_Offset = buffer[1].Trim();
                                        break;
                                    case "KEYBMOUSE_X_NOP_OFFSET_2":
                                        _KeybMouse_X_NOP_Offset_2 = buffer[1].Trim();
                                        break;
                                    case "KEYBMOUSE_Y_NOP_OFFSET":
                                        _KeybMouse_Y_NOP_Offset = buffer[1].Trim();
                                        break;
                                    case "KEYBMOUSE_Y_NOP_OFFSET_2":
                                        _KeybMouse_Y_NOP_Offset_2 = buffer[1].Trim();
                                        break;
        ...
        private void SetHack()
        {
            ...
            try
            {
                bTampon = ReadBytes((int)_BasePtr + (0x8 * _DinputNumber), 8);
            ...
            //Nops
            SetNops((int)_TargetProcess_MemoryBaseAddress, _KeybMouse_X_NOP_Offset);
            SetNops((int)_TargetProcess_MemoryBaseAddress,_KeybMouse_X_NOP_Offset_2);
            SetNops((int)_TargetProcess_MemoryBaseAddress, _KeybMouse_Y_NOP_Offset);
            SetNops((int)_TargetProcess_MemoryBaseAddress, _KeybMouse_Y_NOP_Offset_2);
            ...
            {
                WriteBytes(_KeybMouse_BaseAddress + _KeybMouse_X_Offset, new byte[] { 0, 0, 0, 0, 0, 0, 0, 0 });
                WriteBytes(_KeybMouse_BaseAddress + _KeybMouse_Y_Offset, new byte[] { 0, 0, 0, 0, 0, 0, 0, 0 });
