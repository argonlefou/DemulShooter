...
namespace UnityPlugin_BepInEx_NHA2
{
    class mnew_game_gui_connect_player_slider
    {
        ...
        class update_work_data
        {
            ...
            {
                if (NightHunterArcade2_Plugin.Configurator.RemoveCrosshair && NightHunterArcade2_Plugin.Configurator.RemoveLaser && NightHunterArcade2_Plugin.Configurator.RemoveGuns)
                {
                        ...
                        if (c.name == "di")
                        {
                            ...
                            if (PlayerNum == 1)
                            {
                                if (PlayerGunColor.Contains("RED"))
                                    img.sprite = NightHunterArcade2_Plugin.Sprite_1P_Red;
                                else if (PlayerGunColor.Contains("BLUE"))
                                    img.sprite = NightHunterArcade2_Plugin.Sprite_1P_Blue;
                            }
                            if (PlayerNum == 2)
                            {
                                if (PlayerGunColor.Contains("RED"))
                                    img.sprite = NightHunterArcade2_Plugin.Sprite_2P_Red;
                                else if (PlayerGunColor.Contains("BLUE"))
                                    img.sprite = NightHunterArcade2_Plugin.Sprite_2P_Blue;
                            }
                            break;
                        }
                    }
