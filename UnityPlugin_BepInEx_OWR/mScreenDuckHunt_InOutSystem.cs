...
{    
    class mScreenDuckHunt_InOutSystem
    {
        ...
        class ResetCursorNormalState
        {
            ...
            {
                OpWolf_Plugin.MyLogger.LogMessage("mScreenDuckHunt_InOutSystem.ResetCursorNormalState()");
                ___UIMode = false;
        ...
        class OnUpdate
        {
            static bool Prefix(ScreenDuckHunt_InOutSystem __instance)
            {
                ...
                if (OpWolf_Plugin.PluginControllers[(int)OpWolf_Plugin.PlayerType.Player1].GetButtonDown(PluginController.PluginButton.Trigger))
                {
                    UseMethodFromName(__instance, "FirePressed", new object[] { 0 });
                }
                if (OpWolf_Plugin.PluginControllers[(int)OpWolf_Plugin.PlayerType.Player1].GetButton(PluginController.PluginButton.Trigger))
                {
                    UseMethodFromName(__instance, "FireHold", new object[] { 0 });
                }
                if (OpWolf_Plugin.PluginControllers[(int)OpWolf_Plugin.PlayerType.Player1].GetButtonUp(PluginController.PluginButton.Trigger))
                {
                    UseMethodFromName(__instance, "FireReleased", new object[] { 0 });
                }

                if (OpWolf_Plugin.PluginControllers[(int)OpWolf_Plugin.PlayerType.Player1].GetButtonDown(PluginController.PluginButton.Reload))
                {
                    UseMethodFromName(__instance, "Reload", new object[] { 0 });
                }
                if (OpWolf_Plugin.PluginControllers[(int)OpWolf_Plugin.PlayerType.Player1].GetButtonUp(PluginController.PluginButton.Reload))
                {
                    UseMethodFromName(__instance, "ReloadReleased", new object[] { 0 });
                }

                if (OpWolf_Plugin.PluginControllers[(int)OpWolf_Plugin.PlayerType.Player1].GetButtonDown(PluginController.PluginButton.Action))
                {
                    UseMethodFromName(__instance, "SelectNextWeapon", new object[] { 0 });
                }

                /* Custom Inputs for Player 2 */
                if (OpWolf_Plugin.PluginControllers[(int)OpWolf_Plugin.PlayerType.Player2].GetButtonDown(PluginController.PluginButton.Trigger))
                {
                    UseMethodFromName(__instance, "FirePressed", new object[] { 1 });
                }
                if (OpWolf_Plugin.PluginControllers[(int)OpWolf_Plugin.PlayerType.Player2].GetButton(PluginController.PluginButton.Trigger))
                {
                    UseMethodFromName(__instance, "FireHold", new object[] { 1 });
                }
                if (OpWolf_Plugin.PluginControllers[(int)OpWolf_Plugin.PlayerType.Player2].GetButtonUp(PluginController.PluginButton.Trigger))
                {
                    UseMethodFromName(__instance, "FireReleased", new object[] { 1 });
                }


                if (OpWolf_Plugin.PluginControllers[(int)OpWolf_Plugin.PlayerType.Player2].GetButtonDown(PluginController.PluginButton.Reload))
                {
                    UseMethodFromName(__instance, "Reload", new object[] { 1 });
                }
                if (OpWolf_Plugin.PluginControllers[(int)OpWolf_Plugin.PlayerType.Player2].GetButtonUp(PluginController.PluginButton.Reload))
                {
                    UseMethodFromName(__instance, "ReloadReleased", new object[] { 1 });
                }

                if (OpWolf_Plugin.PluginControllers[(int)OpWolf_Plugin.PlayerType.Player2].GetButtonDown(PluginController.PluginButton.Action))
                {
                    UseMethodFromName(__instance, "SelectNextWeapon", new object[] { 1 });
                }

                //Adding a keyboard key for Grenade P2
                if (Input.GetKeyDown(OpWolf_Plugin.P2_Grenade_KeyCode))
                {
                    UseMethodFromName(__instance, "SelectGrenade", new object[] { 1 });
                }
                if (Input.GetKey(OpWolf_Plugin.P2_Grenade_KeyCode))
                {
                    UseMethodFromName(__instance, "HoldGrenade", new object[] { 1 });
                }
                if (Input.GetKeyUp(OpWolf_Plugin.P2_Grenade_KeyCode))
                {
        ...
        private static void UseMethodFromName(ScreenDuckHunt_InOutSystem Instance, string MethodName, object[] Parameters)
        {
            ...
            {
                OpWolf_Plugin.MyLogger.LogError("Error trying to run method " + MethodName + " from " + Instance.GetType().ToString() + " : " + Ex.Message.ToString());
            }
        }
        ...
        public static void AddEvent(ScreenDuckHunt_InOutSystem Instance, ScreenView CurrentView, string Method, string Event)
        {
            ...
            if (mInfo != null)
            {
                ...
                if (eventInfo != null)
                {
                    ...
                    OpWolf_Plugin.MyLogger.LogMessage("Succesfully added handler '" + Method + "' to event '" + Event + "'");
                }
                else
                    OpWolf_Plugin.MyLogger.LogError("Impossible to find Event " + Event);
            }
            else
                OpWolf_Plugin.MyLogger.LogError("Impossible to find Method " + Method);
        }
            ...
            foreach (MethodInfo m in mInfo)
            {
                if (m.Name.Equals(Method))
                {
                    try
                    {
                        ...
                        {
                            Type eventType = eventInfo.EventHandlerType;
                            OpWolf_Plugin.MyLogger.LogMessage("Found " + m + " of Type " + eventType.ToString());
